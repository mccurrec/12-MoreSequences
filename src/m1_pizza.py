...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(150, 300),  # All numbers are approximate.
                rg.Point(100, 350),
                rg.Point(50, 300),
                rg.Point(100, 250)]
...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'Ezrie wrote this test'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(300, 300), 150)
    circle.fill_color = 'red'
    draw_points_on_circle(window, circle, 65, 'deep pink')
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        line = rg.Line(circle.center, end_points[k])
        line.color = color
...
def run_test_fancy_polygon():
    ...
    # Test 4 (on a new window):
    title = ('FANCY POLYGON test 4: 31 white lines on a black circle, hops = 15')
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(250, 250), 150)
    circle.fill_color = 'white'
    fancy_polygon(window, circle, 31, 15, 'black', 2)
    window.close_on_mouse_click()
